...
const login = async function (event) {
  ...
  console.log(message);
  if (message === "success") {
    // if message says success, redirect user to main page
    window.location.href = "/app.html";
  } else if (message === "Creation Failure") {
    // if message says creation failure, inform user new account not created and encourage them to try again
    const parent = document.getElementById('message');
    const note = document.createElement('p');
    note.textContent = "Account creation failed! Please try again.";
    parent.appendChild(note);
  } else if (message === "Incorrect password") {
    // if message says incorrect password, inform user that password is incorrect and encourage them to try again
    const parent = document.getElementById('message');
    const note = document.createElement('p');
    note.textContent = "Incorrect Password! Login Failed. Please try again.";
    parent.appendChild(note);
  } else if (message === "Login Failed") {
    // if message is general failure, inform user and encourage them to try again
    const parent = document.getElementById('message');
    const note = document.createElement('p');
